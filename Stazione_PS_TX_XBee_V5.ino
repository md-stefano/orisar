...
// stazione passaggio
// V5 settembre 2025
String versione = "OriSar passaggio TX XBee V5 settembre 2025";
byte stazione = 44; // 20----58 la prima e la 4=19 start - l'ultima e la 61=100 -  62=101 e il finish
byte sector = 1;
byte blockAddr = stazione;
...
void loop()
{
  ...
  if (codiceLetto == "9999") // settaggio timer dal master
  {
  ...
  else  // ************scrittura passaggio stazione:ora:minuto:secondo:
  {
    ...
    while (trovato != 's')
    {
      ...
      String buffer1;
      for (byte h = 0; h < 16; h++) buffer1 += buffer[h];
      stazione1 = buffer1.substring(0, 2);
      if (stazione1 == "")
        trovato = 's';

    }
    if (String(stazione) == precedente)
      i--;
    if (i == 63) return;
    if (i == 3 || i == 7 || i == 11 || i == 15 || i == 19 || i == 23 || i == 27 || i == 31 || i == 35 || i == 39 || i == 43 || i == 47 || i == 51 || i == 55 || i == 59)
    {
    ...
    sprintf(buffer1,  "%02d:%02d:%02d:%02d", stazione, now.hour(), now.minute(), now.second());

    for (byte i = 12; i < 16; i++) buffer1[i] = ' ';
    status1(blockAddr);
    ...
    char trasmettere[20];
    sprintf(trasmettere,  "%s:%s", buffer1, code1);
